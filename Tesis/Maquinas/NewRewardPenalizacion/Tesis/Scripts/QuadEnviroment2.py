...
class quadEnviSim(gym.Env):
    ...
    #  ----------------------- ACTION  ------------------------
    def movement_action(self, action, distance=0.10):
        ...
        if lado == 'I':
            for i in range(len(imagen)):
                for j in range(len(imagen[0])):
                    if 0<=j<=105:
                        if i+10<128 and j+10<256:
                            if (imagen[i][j]!=255 and imagen[i][j]>0) and (imagen[i][j+10]!=255 and imagen[i][j+10]>0) and (imagen[i+10][j+10]!=255 and imagen[i+10][j+10]>0) and (imagen[i+10][j]!=255 and imagen[i+10][j]>0):                    
    ...
    def get_reward(self, prevState, newState, step):
        ...
        if  ((newState[7]-0.30)<=newState[0]<=newState[7]+0.30) and ((newState[8]-0.30)<=newState[1]<=newState[8]+0.30):
        ...
        # Verify if collision with obstacle
            ...
            if ((newState[2]<0.4 and prevState[2]<newState[2]) or 
                (newState[3]<0.4 and prevState[3]<newState[3]) or 
                (newState[4]<0.4 and prevState[4]<newState[4]) or
                (newState[5]<0.4 and prevState[5]<newState[5]) ):
                reward-= 0.2
            else:
                reward += -1.5
...
def main():
    ...
    path_tensorBoard = 'C:/Users/KSCC/Documents/Tesis/results/logs/' 
    #tensorboard --logdir ./ --host=127.0.0.1
    # Create log dir
    log_dir = "C:/Users/KSCC/Documents/Tesis/results/logs/DQN1/"
    # Leaf directory
    ...
    cnn_model = DQN(MlpPolicy, env, tensorboard_log= path_tensorBoard, learning_rate=0.0001, buffer_size=1000000,
                    learning_starts=40000, batch_size=32, optimize_memory_usage=False, target_update_interval=5000,
    ...
    cnn_model.learn(total_timesteps=300000, callback=callback, tb_log_name= datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
    cnn_model.save('C:/Users/KSCC/Documents/Tesis/results/logs/EntrenamientoCompletado/'+ datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
